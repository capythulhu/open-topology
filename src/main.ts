...
import * as align from './effects/align.slang';
...
const EFFECTS: Record<string, SlangModule> = { contours, measure, cage, domes, align, tomography, wind, water, sparks, normals, raw };
...
async function main() {
  ...
  const frame = () => {
    ...
    if (projector && !projector.closed && screen && screenContext && projectorDepth && warp) {
      ...
      values[11] = warpState.offsetX / fieldMm;
      values[12] = warpState.offsetY / fieldMm;
