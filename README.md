...
Early, but it runs. Live Kinect depth or animated noise, in 3D, with eleven effects — including a shallow-water simulation, a wind tunnel, one that measures each object in mm, cm², and mL, and one that boxes each object and reports its dimensions. Every panel setting is saved to a local file and restored on the next start. A projector window shows the effect top-down; drag it onto the projector display once and press `F` — it reopens there next time. A small alignment widget in the panel offsets, stretches, rotates, and mirrors the image onto the sand, and the projector's position in millimetres corrects perspective on raised objects; four-corner keystone calibration is not built yet.
